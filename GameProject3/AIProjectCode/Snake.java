...
class Snake {
    ...
    Snake(int layers)                           //makes a new snake with provided amount of hidden layers in neural network
    {
      ...
      if(!humanPlaying) 
      {
//IMPORTANT FOR INPUT NODE CREATION. THIS USES VISION, CAN USE A DIFFERENT ONE
...
//IMPORTANT: to make the AI
        brain = new NeuralNet(24,hidden_nodes,4,layers);            ////makes a neural net brain, in(vision), hidden nodes, out decision size, hidden layers
        
        body.add(new PVector(800,(height/2)+SIZE));                 //this starts the snake in the top corner 
        body.add(new PVector(800,(height/2)+(2*SIZE)));
        score+=2;
      }
    }
//MAYBE IMPORTANT: MAKES A REPLAY OF THE GAME 
...
//IMPORTANT FOR CHECKING BODY COLLISION ON LOOKAHEAD
...
//IMPORTANT CHECKS FOOD COLLISION 
...
//IMPORTANT: CHECKS WALL COLLISION
...
//portentially useful
    void eat() {  //eat food
      ...
      score++;
      if(!humanPlaying && !modelLoaded) {
        if(lifeLeft < 500) {          //ends game in 500 useless moves, tops up with each eat action
          if(lifeLeft > 400) {
             lifeLeft = 500; 
          } else {
            lifeLeft+=100;
      ...
      if(len >= 0) {
        body.add(new PVector(body.get(len).x,body.get(len).y)); //adds new body segment at last element in body list
      } else {
...
//potentially useful
    ...
    Snake crossover(Snake parent) {  //crossover the snake with another snake
       Snake child = new Snake(hidden_layers);          //clones snake and crosses with parent
...
//IMPORTANT: checks surroundings    
    ...
    float[] lookInDirection(PVector direction) {  //look in a direction and check for food, body and wall
      ...
      pos.add(direction);
      distance +=1;
      while (!wallCollide(pos.x,pos.y)) {               //while not hitting wall, check food and body collision and report 1 if hit in corresponding index of look
