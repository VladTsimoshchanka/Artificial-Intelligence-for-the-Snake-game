...
class Matrix 
{
    ...
    float[][] matrix;
     ...
     void AdamOptimize(float learnRate)         //source: https://github.com/sagarvegad/Adam-optimizer/blob/master/Adam.py
     {
        ...
        float movingAvg = 0;
        float sqMovingAvg = 0;
        float epsilon = Math.pow(1, -8);                    //not sure if needs to be a parameter or not
         ...
         for(int j = 0; j < cols; j++) 
         {
            int t = 0;
               while(true)
               {
                  ...
                  float movingBiasCorrectedEstimate = movingAvg/(1-(beta1**t));		//calculates the bias-corrected estimates
                  float sqMovingBiasCorrectedEstimate = sqMovingAvg/(1-(beta2**t));		//calculates the bias-corrected estimates
                  matrix[i][j] = matrix[i][j] - (learnRate*movingBiasCorrectedEstimate)
                                                /(math.sqrt(sqMovingBiasCorrectedEstimate + epsilon);    //updates the parameters
